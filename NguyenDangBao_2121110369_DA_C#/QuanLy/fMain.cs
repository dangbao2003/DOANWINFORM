...
namespace QuanLy
{
    public partial class Form1 : Form
    {
        ...
        private void getHVRowToTxtBOX(DataGridViewRow row)
        {
            ...
            if (DateTime.TryParseExact(row.Cells["ngayhethan"].Value?.ToString(), "M/d/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime dt))
            {
                lblHetHan.Text = dt.ToString("d/M/yyyy");
            }
            else
            {
                lblHetHan.Text = "Ngày không hợp lệ";
            }

            if (row.Cells["hinhanh"].Value != null)
            {
                byte[] data = (byte[])row.Cells["hinhanh"].Value;
                if (data.Length > 0)
                {
